...
class Conv1(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size=1, padding=0, stride=1, dilation=1, groups=1):
...
class RGB(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.sobelconv = Sobelxy(in_channel)
        self.convup = Conv1(in_channel, out_channel)
...
class DenseConv2d(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride):
        ...
        self.dense_conv = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, stride=1)

    def forward(self, input):
        # print(input.shape)[1,16,120,120]
        out = self.dense_conv(input)
        # print(out.shape)#[1,16,118,118]所以dense.shape有问题
...
class MODEL(nn.Module):
    def __init__(self, img_size=120, patch_size=4, embed_dim=96, num_heads=8, window_size=1, in_channel=2,
        ...
        nb_filter = [16, 64, 32, 16, 48]

        self.Convdown = Conv1(nb_filter[1], out_channel)
        self.Convdown1 = Conv1(nb_filter[4], out_channel)
        self.Convup = Conv1(nb_filter[0], out_channel)

        self.RGB = RGB(nb_filter[0], out_channel)

        # dense
        # encoder
        self.DB1 = denseblock(nb_filter[0], kernel_size=3, stride=1)
        # decoder
        self.conv2 = ConvLayer(nb_filter[1], nb_filter[1], kernel_size=3, stride=1)
        self.conv3 = ConvLayer(nb_filter[1], nb_filter[2], kernel_size=3, stride=1)
        self.conv4 = ConvLayer(nb_filter[2], nb_filter[3], kernel_size=3, stride=1)
        # transformer
    ...
    def forward(self, input):
        # print(input.shape)
        convolutional = self.convolutional(input)
        # print(convolutional.shape)
        dense = self.DB1(convolutional)
        dense1 = self.Convdown(dense)
        encode = self.relu(dense1)
        sobel = self.RGB(convolutional)
        encode1 = encode + sobel + convolutional
        cam = self.cam(encode1)
        sam = self.sam(encode1)
        fem_cam = encode1 * cam
        fem_sam = encode1 * sam
        add = fem_cam + fem_sam + encode1
        encode1 = self.relu(add)
        encode1_size = (encode1.shape[2], encode1.shape[3])
        Transformer = self.basicLayer(encode1, encode1_size)
 ...
 #net = MODEL().cuda()
 #input = torch.rand(1,2,120,120).cuda()
 #x = net(input)
 #print(x.shape)
